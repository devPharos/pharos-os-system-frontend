...
export default function CreateOSForm({ serviceOrder, id }: OsFormProps) {
  const [clients, setClients] = useState<Client[]>([])
  const [loading, setLoading] = useState(false)
  const [clientId, setClientId] = useState<string | null>(null)
  const [osDetails, setOsDetails] = useState<ServiceOrderDetails[]>([])
  const [osDetail, setOsDetail] = useState<ServiceOrderDetails>()
  ...
  useEffect(() => {
    if (id) {
      ...
      if (serviceOrder) {
        ...
        setOsDetails(serviceOrder.osDetails)
      }
    }
  ...
  const handleCardDetailClick = (detail: ServiceOrderDetails) => {
    setOsDetail(detail)

    if (serviceOrder) {
      setClientId(serviceOrder?.clientId)
    }
  }
